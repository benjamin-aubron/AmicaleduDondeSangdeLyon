...
import { CollecteProps } from "../components/Collectes";
...
const AssoEnBrefList: AssoProps[] = [
  ...
  },
  {
    ...
    mesure: countCollectes(collectsList).toString(),
    description: "Participations aux collectes",
  }
]

function countCollectes(data: CollecteProps[]) {
  const collects = data.filter(el => el.results !== "");
...
function AssoEnBref({ icon, mesure, description }: AssoProps) {
  return (
    <div className="flex items-center p-6 bg-white dark:bg-[#242526] rounded-2xl shadow-md">
      ...
      <div className="ml-4">
        <div className="text-2xl md:text-3xl font-bold text-red-700 dark:text-slate-200">
          {mesure}
        </div>
